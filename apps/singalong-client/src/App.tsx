...
const PLAYER_USERNAME = import.meta.env.VITE_SINGALONG_PLAYER_USERNAME ?? 'admin'
const PLAYER_PASSWORD = import.meta.env.VITE_SINGALONG_PLAYER_PASSWORD ?? 'password'
const PLAYER_QR_DOMAIN_LABEL = 'singalong.nicenature.space'
const PLAYER_ENCOURAGEMENTS = [
  'Fantastic!',
  'Amazing!',
  'Incredible!',
  'Great job!',
...
function formatDurationClock(valueSeconds: number | null | undefined): string {
...
function isPlayerHostAllowed(hostname: string): boolean {
...
async function fetchActiveSession(): Promise<SessionRecord | null> {
  try {
...
function PlayerPage() {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    void apiJson<LoginResponse>('/users/login', {
      method: 'POST',
      body: JSON.stringify({
      ...
      .then((payload) => {
        if (!cancelled) {
          setPlayerToken(payload.access_token)
        }
      })
      .catch((error) => {
        if (!cancelled) {
  ...
  useEffect(() => {
    ...
    if (currentQueueItem.status === 'playing') {
      // Keep the active item autoplaying after refresh while still restoring its position.
      resumeIsPlayingRef.current = true
  ...
  }, [activeSessionCode, applyPlayerVolume, currentQueueItem])
  ...
  }, [activeSessionCode, applyPlayerVolume, hostAllowed, playerToken])
  ...
  }, [activeSessionCode, activeMediaSrc])
    ...
    <main className="player-screen" ref={playerContainerRef}>
      ...
      <section className="player-overlay player-overlay-top">
        <div className="player-queue-marquee" ref={queueViewportRef}>
          ...
          {pendingQueueItems.length === 0 ? (
            <p className="player-queue-empty">No queued songs yet.</p>
          ) : (

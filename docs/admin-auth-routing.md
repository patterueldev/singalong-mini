...
## Routes
- `/client/admin/login` shows the admin login form.
- `/client/admin/sessions` shows the authenticated session management screen.
- `/client/admin/sessions/:sessionCode` shows the session control screen for a specific active session.
- `*` redirects to `/client/admin/login` when logged out and `/client/admin/sessions` when authenticated.
- The production app is served under `/client/`, so the router must use the Vite base path.
